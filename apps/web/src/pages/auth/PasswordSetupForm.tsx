...
export const PasswordSetupForm: React.FC<{ endpoint: "/auth/activate" | "/auth/reset-password"; successMessage: string }> = ({
  ...
  if (!token) {
    return <p className="text-sm text-red-300">This link is missing its token. Please use the link from your email exactly as sent.</p>;
  }

  if (done) {
    return <p className="text-sm text-emerald-300">{successMessage} Redirecting you to sign in…</p>;
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      ...
      <div>
        <Label htmlFor="confirm" required className="!text-slate-200">
          Confirm password
        </Label>
        <PasswordInput id="confirm" error={errors.confirm?.message} {...register("confirm")} />
      </div>
      {serverError && (
        <div className="rounded-lg border border-red-400/30 bg-red-500/10 px-3 py-2 text-sm text-red-300">{serverError}</div>
      )}
