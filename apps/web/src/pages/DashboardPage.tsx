...
function StatCard({
  ...
  return (
    <Card
      ...
      className={clsx("p-4 text-left", onClick && "cursor-pointer text-left transition-shadow hover:shadow-md focus:outline-none focus:ring-2 focus:ring-brand-500")}
    >
        ...
        <div className="max-h-[60vh] overflow-y-auto overflow-x-auto rounded-xl border border-slate-200">
          {tasks.map((t: any, idx: number) => (
            <button
              ...
              className={clsx(
                "flex w-full flex-wrap items-center gap-3 px-3 py-2.5 text-left hover:bg-slate-50 sm:flex-nowrap",
...
export default function DashboardPage() {
  ...
  const { data: boards } = useBoards({ scope: "MY" });
    ...
    <div className="space-y-6">
      ...
      {data && (
        <>
          ...
          <Card className="p-4">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-800">My Work Time</p>
              <span
                className={clsx(
              ...
              >
                <span className={clsx("h-1.5 w-1.5 rounded-full", todayTime?.isRunning ? "bg-emerald-500" : "bg-slate-400")} />
                {todayTime?.isRunning ? "Tracking" : "Paused"}
              </span>
            </div>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-xl font-semibold text-slate-900">{formatDuration(todayTime?.todaySeconds ?? 0)}</p>
                <p className="text-xs text-slate-500">Today</p>
              </div>
              <div>
                <p className="text-xl font-semibold text-slate-900">{formatDuration(weekTime?.totalSeconds ?? 0)}</p>
                <p className="text-xs text-slate-500">This Week</p>
              </div>
              <div>
                <p className="text-xl font-semibold text-slate-900">{formatDuration(monthTime?.totalSeconds ?? 0)}</p>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <Card className="p-4">
              <p className="mb-3 text-sm font-semibold text-slate-800">Tasks by Status</p>
              <div className="relative">
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={data.statusDistribution} dataKey="count" nameKey="stage" innerRadius={55} outerRadius={80} paddingAngle={2}>
              ...
              <div className="mt-1 flex flex-wrap justify-center gap-3">
                {data.statusDistribution.map((s: any, idx: number) => (
                  <div key={`${s.stage}-${idx}`} className="flex items-center gap-1.5 text-xs text-slate-600">
                    <span className="h-2 w-2 rounded-full" style={{ background: STATUS_PALETTE[idx % STATUS_PALETTE.length] }} />
                    {s.stage} ({s.count})
                  </div>
                ))}
              </div>
              ...
              {data.recentActivity.map((a: any) => (
                <li key={a.id} className="flex items-center justify-between py-2 text-sm">
                  <span className="text-slate-600">
                    <span className="font-medium text-slate-900">{a.actorName}</span> {a.action.toLowerCase()}d a {a.entityType.toLowerCase()}
                  </span>
                  <span className="shrink-0 text-xs text-slate-400">{formatDistanceToNow(new Date(a.createdAt), { addSuffix: true })}</span>
                </li>
              ))}
            </ul>
          </Card>
